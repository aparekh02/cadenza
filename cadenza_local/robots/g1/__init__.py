...
"existing primitive library" (CLAUDE.md: cadenza/robots/g1/).

It re-exports the action definitions, MuJoCo model, robot spec, controller,
and gait engine so every consumer — examples, tests, gym3d, and the C++ OS
wrapper — accesses the same objects from one canonical location.
"""
...
def get_gait_engine():
    ...
    import cadenza_local.g1_gait as gait
    return gait
