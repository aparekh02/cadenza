...
class Go1:
    ...
    def run(self, sequence: list):
        ...
        """
        from cadenza_local.sim import Sim
        ...
        with mujoco.viewer.launch_passive(sim.model, sim.data) as viewer:
            ...
            for i, item in enumerate(steps):
                ...
                if isinstance(item, list):
                    ...
                    if item.extension != 1.0:
                        print(f"  ext={item.extension}", end="")
                    print()
    ...
    def _execute_single(self, step: Step, sim, lib, viewer):
        """Execute one Step."""
        ...
        if action_spec.is_gait:
            ok = sim._run_gait(action_spec, viewer, call)
        elif action_spec.is_phase:
            ok = sim._run_phase(action_spec, viewer, call)
        else:
            ok = True
