...
def selectRobot(num,vel,step_len,stairs_height,spno):
    ...
    if num==1: #Kondo khr3hv
        ...
        # xml_path = os.path.join(dirname + "/" + xml_path)
        # xml_tree = ET.parse(xml_path) 
        # Find G1 model: cadenza/models/g1/g1_position.xml (up 2 levels from zmp/)
        _models_dir = os.path.join(dirname, '..', '..', 'models', 'g1')
        robotpath = os.path.join(_models_dir, 'g1_position.xml')
